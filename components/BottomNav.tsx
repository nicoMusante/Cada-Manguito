...
export function BottomNav({ t, active, onChange }: { t: Theme; active: TabId; onChange: (id: TabId) => void }) {
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 backdrop-blur px-6 py-3 flex items-center justify-around border-t" style={{ backgroundColor: t.navBg, borderColor: t.divider }}>
      {TABS.map((tab) => (
        <button key={tab.id} onClick={() => onChange(tab.id)} className="flex flex-col items-center gap-1">
          ...
          <span className="text-[10px]" style={{ color: active === tab.id ? t.textPrimary : t.textSecondary }}>{tab.label}</span>
        </button>
      ))}
    </div>
  );
