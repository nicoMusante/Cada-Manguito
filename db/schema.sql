...
-- Intentos fallidos de login/registro, para rate limiting (ver lib/rateLimit.ts).
...
-- cuotas vía pagos_deuda), no el monto original. Ojo: no filtro por
-- "neto != 0" — si alguien tiene una deuda ME_DEBEN y otra YO_DEBO que se
-- compensan exacto, el neto da 0 pero las dos entradas siguen pendientes de
-- verdad (no se saldan solas) y la persona tiene que seguir apareciendo acá
